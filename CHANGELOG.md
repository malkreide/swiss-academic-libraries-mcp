...
- **The retry had six defects, all inherited from the shared template.** This
  ...
  6. **Point six did not apply here.** `http_get_with_retry` already ended in
     `raise last_error` rather than wrapping — the caller keeps the exception
     type and `.response`. The per-attempt log line did interpolate the
     exception alone (`err=%s`), and `httpx.ConnectTimeout`, `ReadTimeout` and
     `ConnectError` carry an **empty** `str()`, so `err=` ended in nothing for
     exactly the failures an outage produces; it now names the type too.
